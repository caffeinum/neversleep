...
Loop escalates through a 10-rung ladder that **interleaves modes** (engineering / product-PM / UI-UX / TRIZ-scope / LSD-brainstorm) so consecutive passes feel different: run-it → user → edge-cases → friction → scope → correctness → value-prop → delight → moonshot → senior-eng, cycling endlessly. It **leads with actually running the built software** (deliberate stance) and pushes subagents + ultracode over solo grind. Rungs generated via a 5-persona workflow + synthesis. The hook **always** blocks — claude never voluntarily stops. The only off-ramp is ctrl-c (never trapped; bypasses hooks). No sentinel, no max-passes ceiling. Per-session pass counter (just for rung cycling/display) lives in `$TMPDIR/neversleep-<session_id>.json`.
