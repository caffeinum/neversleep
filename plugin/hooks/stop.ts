#!/usr/bin/env bun
// anxiety plugin — Stop hook.
// Unlike the neversleep CLI (which always blocks), the plugin's loop is OPT-IN per
// session: it only blocks when this session's toggle file exists. `/anxiety` (or
// `/anxiety on`) creates it; `/anxiety off` removes it. Keyed by session_id so it's
// scoped to the session you turned it on in — nothing else loops.
//
// Stop-hook contract: print {"decision":"block","reason":...} to keep claude going,
// or {} to let it stop. ctrl-c always bypasses hooks.
...
// keyed by session_id — must match the sanitization in commands/toggle.sh
const sessionId = String(payload?.session_id ?? "default").replace(/[^A-Za-z0-9_-]/g, "");
const toggleFile = join(tmpdir(), `anxiety-on-${sessionId}`);

// toggle OFF (or never set) → let claude stop normally
if (!(await Bun.file(toggleFile).exists())) {
  process.stdout.write("{}");
  process.exit(0);
}

// toggle ON → loop. per-session pass counter for rung cycling.
const stateFile = join(tmpdir(), `anxiety-${sessionId}.json`);
let passes = 0;
...
// emit block before persisting — the loop must never depend on the write
process.stdout.write(JSON.stringify({
  decision: "block",
  reason: `[anxiety · ${rung.stage} · pass ${passNo}] ${rung.reason}`,
}));
